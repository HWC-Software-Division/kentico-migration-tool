        ...
        foreach (var ksSite in sites)
        {
            ...
            for (int pass = 0; pass < 2; pass++)
            {
                ...
                while (linkTopoSortedNodes.GetNext(out var ksTreeDefItem))
                {
                    ...
                    if (migratedDocuments.Count == 0)
                    {
                        logger.LogWarning(
                            "Skipping node '{NodeAliasPath}' (NodeID={NodeID}, Class={ClassName}) because no CMS_Document record exists.",
                            ksNode.NodeAliasPath,
                            ksNode.NodeID,
                            nodeClassClassName);
                ...
                ksTrees = deferredTreeNodesService.GetNodes().ToList();
            }
