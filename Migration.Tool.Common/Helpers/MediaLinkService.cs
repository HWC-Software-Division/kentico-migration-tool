    ...
    public MatchMediaLinkResult MatchMediaLink(string? linkStr, int? currentSiteId)
    {
        ...
        if (site != default && uri is not null)
        {
            if (Uri.IsWellFormedUriString(site.siteLiveSiteUrl, UriKind.Absolute))
            {
                ...
                if (uri.LocalPath.StartsWith(siteLiveSiteUri.LocalPath, StringComparison.InvariantCultureIgnoreCase))
                {
                    ...
                    if (spl.Length == 0 || inspectionIndex < 0 || inspectionIndex >= spl.Length)
                    {
                        Console.WriteLine(
                            $"[MediaLink parse skipped] linkStr='{linkStr}', path='{path}', inspectionIndex={inspectionIndex}, splLen={spl.Length}, siteId={linkSiteId}"
                        );
        ...
        if (siteMediaFolder && currentSiteId is not null)
        {
            ...
            if (sites.FirstOrDefault(x => x.siteId == linkSiteId) is var (_, siteName, _)
                ...
                && inspectionIndex >= 0
                && inspectionIndex < spl.Length
                && siteName.Equals(spl[inspectionIndex], StringComparison.InvariantCultureIgnoreCase))
            {
                Console.WriteLine(
                    $"[MediaLink parse skipped] inspectionIndex={inspectionIndex}"
                );
